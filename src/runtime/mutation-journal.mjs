...
export class MutationJournal {
  ...
  async undoLatest({ sessionId, projectRoot }) {
    ...
    const target = await resolveWorkspacePath(projectRoot, entry.path, false);
    ...
    if (entry.before.exists) {
      ...
      await writeFile(target.absolute, entry.before.content, 'utf8');
    } else {
...
async function snapshotFile(path) {
  try {
    ...
    return { exists: true, hash: hash(content), content };
  } catch (error) {
    if (error?.code === 'ENOENT') return { exists: false, hash: null, content: '' };
    throw error;
  }
}
