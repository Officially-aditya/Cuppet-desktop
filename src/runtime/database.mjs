...
export class ConversationDatabase {
  ...
  createProject({ id, name, canonicalPath, repositoryId = null, remoteUrl = null, now = Date.now() }) {
    this.#db.prepare(`INSERT INTO projects (id,name,canonical_path,repository_id,remote_url,created_at,updated_at,last_opened_at) VALUES (?,?,?,?,?,?,?,?)`)
      .run(id,name,canonicalPath,repositoryId,remoteUrl,now,now,now);
  ...
  createSession({ id, title='New chat', projectId=null, now=Date.now() }) {
    ...
    this.#db.prepare('INSERT INTO sessions (id,title,created_at,updated_at,project_id) VALUES (?,?,?,?,?)').run(id,title,now,now,projectId);
    if(projectId) this.touchProject(projectId,now);
    return this.getSessionSummary(id);
  }
  listSessions({ projectId }={}) {
    const filter = projectId === undefined ? '' : 'WHERE s.project_id IS ?';
