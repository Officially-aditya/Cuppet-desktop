...
/**
 * Concrete local-CLI lifecycle operations.
 *
 * detect() and probe() are observational only. They never install software or
 * launch authentication UI. install()/update()/authenticate() are explicit
 * mutations, so status checks cannot surprise the user with setup side effects.
 */
export function localProviderOperations(providerID, { userData, runImpl = run, platform = process.platform, now = () => Date.now() } = {}) {
  ...
  const detect = async () => {
    ...
    const ownedByCuppet = state.installedByCuppet === true;
    return {
      providerID: descriptor.id,
      label: descriptor.label,
      installed: true,
      ...
      installation: normalizeProviderInstallation({
        ...
        executable,
        version,
        source: ownedByCuppet ? state.installSource : 'unknown',
        ownedByCuppet,
        canUpdate: ownedByCuppet && Boolean(updateSpec(descriptor.id, platform, state.installSource || spec?.source)),
      }),
    };
  };
  ...
  const install = async () => {
    ...
    await markInstalled(userData, descriptor.id, { source: spec.source, installedAt: now() });
    extendCliSearchPath();
    const after = await detect();
    if (!after.installed) throw new Error(`${descriptor.label} installation completed but Cuppet still could not find the CLI.`);
    return after;
  };
  ...
  const update = async () => {
    ...
    await runImpl(spec.command, spec.args, RUN_TIMEOUT_MS, { stdin: 'ignore', env: spec.env });
    await markInstalled(userData, descriptor.id, { source: detected.installation.source, installedAt: now(), updatedAt: now() });
    return detect();
  };
...
async function markInstalled(userData, providerID, patch) {
  if (!userData) return;
  await patchProviderState(userData, providerID, { installedByCuppet: true, installSource: patch.source || 'managed', installedAt: patch.installedAt, ...(patch.updatedAt ? { updatedAt: patch.updatedAt } : {}) });
}
async function patchProviderState(userData, providerID, patch) {
  const path = join(userData, LINK_STATE_FILE);
  let parsed = { version: 2, providers: {} };
  try {
    const current = JSON.parse(await readFile(path, 'utf8'));
    if (current && typeof current === 'object') parsed = { version: 2, providers: { ...(current.providers ?? {}) } };
  } catch {}
