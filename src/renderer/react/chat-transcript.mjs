...
export function hydrateTranscript(rows = []) {
  let state = {};
  const ordered = Array.isArray(rows)
    ? rows.slice().sort(compareActivityRows)
    : [];
...
export function reduceTranscriptEvent(state, event) {
...
export function orderedTranscriptItems(items = []) {
  return items
...
function appendReasoning(items, segment, sequence) {
...
function updateTool(items, activity, sequence) {
  ...
  const status = closed ? (activity.status === 'success' ? 'complete' : 'error') : 'running';
  const patch = {
    id,
    type: 'tool',
    status,
...
function updateMessageState(state, messageId, updater) {
...
function compareActivityRows(left, right) {
  ...
  const timeCompare = Number(left?.createdAt ?? 0) - Number(right?.createdAt ?? 0);
