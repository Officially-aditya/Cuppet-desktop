const els = Object.fromEntries([
  ['projectList','#project-list'],['newChat','#new-chat'],['addProject','#add-project'],['title','#conversation-title'],['projectStatus','#project-status'],['runtimeStatus','#runtime-status'],['providerPill','#provider-pill'],['modeToggle','#mode-toggle'],['orchestratorToggle','#orchestrator-toggle'],['backgroundToggle','#background-toggle'],['tstPill','#tst-pill'],['messages','#messages'],['composer','#composer'],['prompt','#prompt'],['send','#send-button'],['stop','#stop-button'],['settingsButton','#settings-button'],['settingsDialog','#settings-dialog'],['settingsForm','#settings-form'],['settingsClose','#settings-close'],['settingsCancel','#settings-cancel'],['providerId','#provider-id'],['baseUrl','#provider-base-url'],['model','#provider-model'],['primaryEffort','#provider-primary-effort'],['backgroundModel','#provider-background-model'],['secondaryEffort','#provider-secondary-effort'],['apiKey','#provider-api-key'],['settingsNote','#settings-note'],['toast','#toast'],['newChatDialog','#new-chat-dialog'],['newChatForm','#new-chat-form'],['newChatProject','#new-chat-project'],['newChatClose','#new-chat-close'],['newChatCancel','#new-chat-cancel'],['addProjectDialog','#add-project-dialog'],['addProjectClose','#add-project-close'],['localProjectName','#local-project-name'],['addLocalProject','#add-local-project'],['githubUrl','#github-url'],['cloneUrlProject','#clone-url-project'],['githubSearch','#github-search'],['loadGithubRepos','#load-github-repos'],['githubRepoResults','#github-repo-results'],['addProjectNote','#add-project-note'],
...
function renderSidebar() {
  ...
  for (const project of state.projects) {
    ...
    wrap.append(row); for (const session of state.sessions.filter((item)=>item.projectId===project.id)) wrap.append(sessionButton(session)); children.push(wrap);
  }
...
function renderConversation() {
  ...
  els.title.textContent=view.title||'New chat'; const project=state.projects.find((p)=>p.id===view.projectId);
...
function renderCognitive(){els.modeToggle.textContent=state.sessionMode==='plan'?'Plan':'Build';els.modeToggle.classList.toggle('active',state.sessionMode==='plan');els.orchestratorToggle.textContent=state.cognitive.orchestratorEnabled?'Orchestrator on':'Orchestrator off';els.orchestratorToggle.classList.toggle('active',state.cognitive.orchestratorEnabled);els.backgroundToggle.textContent=state.cognitive.backgroundPaused?'Background paused':'Background on';els.backgroundToggle.classList.toggle('active',!state.cognitive.backgroundPaused);const tst=state.cognitive.tst||{};els.tstPill.textContent=tst.connected?'TST connected':tst.configured?'TST ready':'TST off';}
...
function handleRuntimeEvent(event) {
  ...
  if(event.type==='run.started')state.runningSessions.add(event.sessionId); if(event.type==='run.finished')state.runningSessions.delete(event.sessionId);
  if(event.type==='pe3.routed'&&event.targetSessionId&&event.targetSessionId!==event.sourceSessionId){state.runningSessions.delete(event.sourceSessionId);const action=event.action==='reactivate'?'Resumed earlier task context':'Started a separate task context';toast(`${action}.`);void openSession(event.targetSessionId).catch((error)=>toast(error.message||String(error)));}
  if(event.type==='runtime.error')toast(event.message||'Runtime error'); if(event.type==='cognitive.updated'&&event.cognitive){state.cognitive=event.cognitive;renderCognitive();} if(event.type==='context.compiled'&&event.tst){state.cognitive.tst=event.tst;renderCognitive();}
  if(event.project) upsertProject(event.project); if(event.type==='project.removed'){state.projects=state.projects.filter((p)=>p.id!==event.projectId);if(state.selectedProjectId===event.projectId)state.selectedProjectId=null;void refreshData();}
  if(event.session)upsertSession(event.session); if(event.message&&state.active?.id===event.message.sessionId)upsertActiveMessage(event.message);
...
function showProjectError(error){els.addProjectNote.textContent=error.message||String(error);} function setProjectBusy(busy,message=''){els.addProjectNote.textContent=message;els.addLocalProject.disabled=busy;els.cloneUrlProject.disabled=busy;els.loadGithubRepos.disabled=busy;}
...
async function openSettings(){
  ...
  els.providerId.value=state.provider.providerID||state.provider.primary?.providerID||'openai-compatible';
  els.baseUrl.value=state.provider.baseUrl||'https://api.openai.com/v1';
  els.model.value=state.provider.primary?.modelID||state.provider.model||'';
  els.backgroundModel.value=state.provider.secondary?.modelID||state.provider.backgroundModel||'';
  renderEffortSelect(els.primaryEffort,effortsForModel(els.providerId.value,els.model.value),state.provider.primary?.variant||'');
  renderEffortSelect(els.secondaryEffort,effortsForModel(els.providerId.value,els.backgroundModel.value||els.model.value),state.provider.secondary?.variant||'');
  els.apiKey.value='';els.apiKey.placeholder=state.provider.apiKeyConfigured?'Saved securely · leave blank to keep it':'API key';
  els.settingsNote.textContent=state.provider.encryptionAvailable?'API keys are encrypted with the operating system credential store. Primary drives foreground work; secondary drives background/worker work. Effort is sent as request metadata.':'OS credential encryption is unavailable. Cuppet will refuse to persist an API key in plaintext.';
  els.settingsDialog.showModal();
}
async function saveSettings(event){event.preventDefault();try{state.provider=await window.cuppet.settings.save({providerID:els.providerId.value,baseUrl:els.baseUrl.value,model:els.model.value,primaryEffort:els.primaryEffort.value,backgroundModel:els.backgroundModel.value,secondaryEffort:els.secondaryEffort.value,apiKey:els.apiKey.value});renderProvider();els.settingsDialog.close();toast('Provider roles and effort saved.');}catch(error){els.settingsNote.textContent=error.message||String(error);}}
function refreshEfforts(){renderEffortSelect(els.primaryEffort,effortsForModel(els.providerId.value,els.model.value),els.primaryEffort.value);renderEffortSelect(els.secondaryEffort,effortsForModel(els.providerId.value,els.backgroundModel.value||els.model.value),els.secondaryEffort.value);}
function effortsForModel(providerID,modelID){const normalizedProvider=String(providerID||'').trim().toLowerCase();const model=(state.provider?.models||[]).find((item)=>String(item.providerID||'').toLowerCase()===normalizedProvider&&item.modelID===String(modelID||'').trim());return model?.variants||[];}
function renderEffortSelect(select,values,current){const options=[option('','Default model effort'),...values.map((value)=>option(value,value))];select.replaceChildren(...options);select.disabled=values.length===0;if(current&&values.includes(current))select.value=current;else select.value='';}
function renderProvider(){const selected=state.provider?.primary;els.providerPill.textContent=state.provider?.apiKeyConfigured&&selected?.modelID?`${state.provider.providerID||selected.providerID} · ${selected.modelID}${selected.variant?` · ${selected.variant}`:''}`:'Provider not configured';}
function toast(message){els.toast.textContent=message;els.toast.classList.remove('hidden');clearTimeout(toast.timer);toast.timer=setTimeout(()=>els.toast.classList.add('hidden'),4500);} function resizePrompt(){els.prompt.style.height='auto';els.prompt.style.height=`${Math.min(180,Math.max(54,els.prompt.scrollHeight))}px`;}
...
els.providerId.addEventListener('input',refreshEfforts);els.model.addEventListener('input',refreshEfforts);els.backgroundModel.addEventListener('input',refreshEfforts);
void init();