const els = Object.fromEntries([
  ['projectList','#project-list'],['newChat','#new-chat'],['addProject','#add-project'],['title','#conversation-title'],['projectStatus','#project-status'],['runtimeStatus','#runtime-status'],['providerPill','#provider-pill'],['messages','#messages'],['composer','#composer'],['prompt','#prompt'],['send','#send-button'],['stop','#stop-button'],['settingsButton','#settings-button'],['settingsDialog','#settings-dialog'],['settingsForm','#settings-form'],['settingsClose','#settings-close'],['settingsCancel','#settings-cancel'],['baseUrl','#provider-base-url'],['model','#provider-model'],['apiKey','#provider-api-key'],['settingsNote','#settings-note'],['toast','#toast'],['newChatDialog','#new-chat-dialog'],['newChatForm','#new-chat-form'],['newChatProject','#new-chat-project'],['newChatClose','#new-chat-close'],['newChatCancel','#new-chat-cancel'],['addProjectDialog','#add-project-dialog'],['addProjectClose','#add-project-close'],['localProjectName','#local-project-name'],['addLocalProject','#add-local-project'],['githubUrl','#github-url'],['cloneUrlProject','#clone-url-project'],['githubSearch','#github-search'],['loadGithubRepos','#load-github-repos'],['githubRepoResults','#github-repo-results'],['addProjectNote','#add-project-note'],
].map(([key,selector]) => [key,document.querySelector(selector)]));

const state = { projects: [], sessions: [], active: null, draft: null, selectedProjectId: null, runningSessions: new Set(), provider: null, githubRepos: [] };

async function init() {
  window.cuppet.onEvent(handleRuntimeEvent);
  try {
    const [health, provider, projects, sessions] = await Promise.all([window.cuppet.health(), window.cuppet.settings.get(), window.cuppet.projects.list(), window.cuppet.sessions.list()]);
    state.provider = provider; state.projects = projects; state.sessions = sessions;
    ...
    renderProvider(); renderSidebar(); renderProjectOptions();
...
function startDraft(projectId) {
  state.active = null; state.draft = { projectId: projectId || null, title: 'New chat', messages: [] }; state.selectedProjectId = projectId || null;
  renderSidebar(); renderConversation(); els.prompt.focus();
}

async function openNewChatDialog() {
  renderProjectOptions(); els.newChatProject.value = state.selectedProjectId || ''; els.newChatDialog.showModal();
}

async function createPersistedSessionForDraft() {
  if (!state.draft) return state.active;
  const session = await window.cuppet.sessions.create(state.draft.projectId);
  upsertSession(session); state.draft = null; state.active = { ...session, messages: [] }; state.selectedProjectId = session.projectId || null;
  renderSidebar(); return state.active;
}

async function openSession(id) {
  const session = await window.cuppet.sessions.get(id); state.active = session; state.draft = null; state.selectedProjectId = session.projectId || null;
...
function renderSidebar() {
  ...
  if (general.length) {
    const group=document.createElement('section');group.className='project-group general-group';const label=document.createElement('div');label.className='general-label';label.textContent='General';group.append(label);for(const session of general)group.append(sessionButton(session));children.push(group);
  }
...
function renderRunState(){const running=Boolean(state.active?.id&&state.runningSessions.has(state.active.id));els.stop.classList.toggle('hidden',!running);els.send.disabled=running;els.prompt.disabled=running;}
...
async function openSettings(){state.provider=await window.cuppet.settings.get();els.baseUrl.value=state.provider.baseUrl||'https://api.openai.com/v1';els.model.value=state.provider.model||'';els.apiKey.value='';els.apiKey.placeholder=state.provider.apiKeyConfigured?'Saved securely · leave blank to keep it':'API key';els.settingsNote.textContent=state.provider.encryptionAvailable?'API keys are encrypted with the operating system credential store before persistence.':'OS credential encryption is unavailable. Cuppet will refuse to persist an API key in plaintext.';els.settingsDialog.showModal();}
async function saveSettings(event){event.preventDefault();try{state.provider=await window.cuppet.settings.save({baseUrl:els.baseUrl.value,model:els.model.value,apiKey:els.apiKey.value});renderProvider();els.settingsDialog.close();toast('Provider settings saved.');}catch(error){els.settingsNote.textContent=error.message||String(error);}}
