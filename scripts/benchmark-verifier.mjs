...
async function verify(task, workspace) {
  ...
  if (task === 'quiz-game-greenfield') return verifyWebProject(workspace, 'quiz-game', {
    ...
    scripts: ['questions', 'score', 'addEventListener'], minimumRecords: 5,
  })
  ...
  if (task === 'long-tool-use-dashboard') return verifyWebProject(workspace, 'operations-dashboard', {
    ...
    scripts: ['records', 'addEventListener', 'filter', 'empty'], minimumRecords: 8,
  })
  throw new Error(`no deterministic verifier registered for ${task}`)
}

async function verifyTaskTracker(workspace) {
  const root = join(workspace, 'games', 'task-tracker')
  const files = [...await textFiles(join(root, 'src'), 'src'), ...await textFiles(join(root, 'test'), 'test')]
  ...
  const packageValue = JSON.parse(await readFile(join(workspace, 'package.json'), 'utf8'))
  ...
  await exec(npmBin(), ['run', 'task-tracker:build', '--silent'], { cwd: workspace, maxBuffer: 4 * 1024 * 1024 })
  const stamp = `?bench=${Date.now()}`
  const dist = join(root, 'dist', 'src')
  const core = await import(`${pathToFileURL(join(dist, 'core', 'task.js')).href}${stamp}`)
  ...
  const factory = await import(`${pathToFileURL(join(dist, 'core', 'taskFactory.js')).href}${stamp}`)
  ...
  const store = await import(`${pathToFileURL(join(dist, 'store', 'taskStore.js')).href}${stamp}`)
  const index = await import(`${pathToFileURL(join(dist, 'store', 'taskIndex.js')).href}${stamp}`)
  store.clearTasks()
...
async function verifyWebProject(workspace, project, expected) {
...
async function verifyMiniCrm(workspace, task) {
...
async function verifyNotes(workspace, project, signals) {
  const text = (await readFile(join(workspace, 'projects', project, 'README.md'), 'utf8')).toLowerCase()
  for (const signal of signals) assert.match(text, new RegExp(escapeRegExp(signal)))
}

async function textFiles(root, prefix = '') {
...
function countRecordSignals(script) { return script.match(/(?:id|title|name|label|category|correct|status)\s*:/g)?.length ?? 0 }
...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 2) {
    ...
    if (!key?.startsWith('--') || !value) throw new Error('usage: --task ID --workspace PATH')
    values.set(key.slice(2), value)
  }
  const task = values.get('task'), workspace = values.get('workspace')
