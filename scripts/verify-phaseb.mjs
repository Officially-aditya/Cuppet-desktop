...
const required = [
  'src/runtime/database.mjs', 'src/runtime/projects.mjs', 'src/runtime/service.mjs',
  'src/main/main.mjs', 'src/preload/preload.cjs', 'src/renderer/index.html', 'src/renderer/app.js',
  'test/database-migration.test.mjs', 'test/database-projects.test.mjs', 'test/projects.test.mjs', 'test/runtime-project-binding.test.mjs',
];
...
expect(text['src/runtime/projects.mjs'].includes("['api','--method','GET','user/repos'"), 'authenticated repository picker missing');
...
const testRun = spawnSync(process.execPath, ['--test', 'test/database-migration.test.mjs', 'test/database-projects.test.mjs', 'test/projects.test.mjs', 'test/runtime-project-binding.test.mjs'], { stdio: 'inherit' });
if (testRun.status !== 0) process.exit(testRun.status ?? 1);
console.log('Phase B gate passed: projects, clone/auth recovery, project-bound chats, migration, and switching semantics verified.');
