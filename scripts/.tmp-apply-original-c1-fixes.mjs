...
async function patch(path, replacements) {
  ...
  for (const [before, after, label] of replacements) {
    if (!text.includes(before)) throw new Error(`missing replacement ${label} in ${path}`);
    text = text.replace(before, after);
  }
  await writeFile(path, text);
}

await patch('src/runtime/service.mjs', [
  [
...
await patch('src/runtime/tool-runtime.mjs', [
  [
...
await patch('src/runtime/journaled-tool-runtime.mjs', [
  [
`        onPaths: async (paths, mutation) => {
...
await patch('src/runtime/tst-edit-batches.mjs', [
  [
