...
const protocolDense = dense['src/runtime/remote/protocol.mjs'];
...
const bridgeDense = dense['src/runtime/remote/bridge.mjs'];
expect(bridgeDense.includes('DEDUPE_CAPACITY=512') && bridgeDense.includes('this.#seen.has(dedupeKey)') && bridgeDense.includes('#connectionId=randomUUID()'), 'bridge replay/connection authority missing');
expect(bridge.includes('client.accept') && bridge.includes('client.reject') && bridge.includes('missing scope') && bridge.includes('duplicate:true'), 'bridge authentication/scope/replay behavior missing');

const commands = text['src/runtime/remote/commands.mjs'];
const commandsDense = dense['src/runtime/remote/commands.mjs'];
...
expect(commandsDense.includes("#providerList(){return[{id:'openai-compatible',name:'OpenAI-compatible',connected:"), 'remote provider projection changed');
expect(!commandsDense.includes('baseUrl:this.#provider.baseUrl'), 'remote provider projection exposes local endpoint details');
...
const managerDense = dense['src/runtime/remote/manager.mjs'];
...
expect(manager.includes('Provider configuration is pushed at ordinary desktop startup') && managerDense.includes('if(this.#bridge)awaitthis.stop().catch'), 'unused remote configuration/shutdown may create persistent state');

const relay = text['src/runtime/remote/relay.mjs'];
const relayDense = dense['src/runtime/remote/relay.mjs'];
...
expect(relay.includes('client.accept') && relay.includes('device.authenticated') && relayDense.includes('room.replay.length=0'), 'relay pre-auth/host replacement isolation missing');
