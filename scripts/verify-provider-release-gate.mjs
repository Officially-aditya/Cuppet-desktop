...
const releaseMode = text(process.env.CUPPET_PROVIDER_V2_RELEASE_MODE);
...
if (eventName === 'push' && releaseMode === 'user-requested-unverified') {
  console.log(`[release-gate] PASS: user explicitly requested an UNVERIFIED Provider V2 candidate for ${commit || '(unknown commit)'}. Authenticated OpenCode smoke has NOT been attested.`);
  process.exit(0);
}

if (eventName !== 'workflow_dispatch') {
  fail('Provider V2 builds must be started manually after the authenticated OpenCode smoke passes, unless an explicitly user-requested unverified candidate is being built.');
}
if (!attestedPass) {
